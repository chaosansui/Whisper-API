...
class AudioProcessor:
    ...
    def estimate_audio_quality(self, audio_np: np.ndarray) -> float:
        """估计音频质量，优化信噪比计算"""
        try:
    ...
    def enhance_telephone_audio(self, audio_np: np.ndarray) -> np.ndarray:
        """增强电话录音质量"""
        try:
            # 动态光谱减法降噪
            freqs, psd = welch(audio_np, fs=self.SAMPLE_RATE, nperseg=1024)
    ...
    def basic_enhance_telephone_audio(self, audio_np: np.ndarray) -> np.ndarray:
        """基础电话录音增强"""
        nyquist = self.SAMPLE_RATE / 2
        lowcut = 100 / nyquist
        highcut = 5000 / nyquist
